    ...
    def action_sale_ok(self):
        ...
        if self.partner_id.credit_limit:
            ...
            for record in self:
                ...
                invoices_ids = self.env['account.move'].search(domain)
                dic = {}
                dic['total'] = 0
                total = 0.0
                for invoice_obj in invoices_ids:
                    dic['total'] += invoice_obj.amount_residual 
                    total = dic['total']
                    #total = float("{:.2f}".format(total))
            
            total_invoices_ids = self.env['account.move'].search([('partner_id.id','=', self.partner_id.id),('invoice_date_due','<',datetime.today())])
            dictionary = {}
            dictionary['total'] = 0
            for invoice in total_invoices_ids:
                dictionary['total'] += invoice.amount_residual 
            ...
            if total > self.partner_id.credit_limit:#available_credit:
                ...
                vals_wiz={
                    ...
                    'due_invoice':str(len(invoices_ids))+ ' Facturas por : '+ str(total), #to_invoice_amount
                    'total_invoices':dictionary['total'],
